...
export const typePageTags = async (filteredPageLinksSet: ({ uuid: string; name: string}  | undefined)[], hopLinksElement: HTMLDivElement) => {
    ...
    for (const pageLink of filteredPageLinksSet) {
        ...
        if ((!PageEntity || PageEntity.length === 0) && (!PageEntityFromProperty || PageEntityFromProperty.length === 0)) continue
        ...
        if (PageEntity.length === 0 && PageEntityFromProperty.length === 0) continue
        //sortする
